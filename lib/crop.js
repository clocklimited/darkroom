...
Crop.prototype.exec = function () {
  ...
  var originalBuffer = Buffer.concat(this.chunks, this.size)
  if (imageFileType.ext !== 'gif') {
    ...
    // this is the rounding grpahicsmagick uses for geometry
    var roundedX1 = Math.ceil(this.crop.x1 - 0.5)
      , roundedY1 = Math.ceil(this.crop.y1 - 0.5)
      , roundedWidth = Math.floor(this.crop.w + 0.5)
      , roundedHeight = Math.floor(this.crop.h + 0.5)
      , crop = roundedX1 + ',' + roundedY1 + '+' + roundedWidth + 'x' + roundedHeight
