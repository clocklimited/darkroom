...
function getGifsicleContext (cb) {
  ...
  if (this.mode === 'fit') {
    ...
    input = new Gifsicle(params.concat('--resize-touch', dimens))
    input.on('data', function (data) { buffers.push(data) })
    input.on('end', function () {
      ...
      gm(resized).identify(function (err, data) {
        ...
        context.write(resized)
        context.end()
        cb(null, context)
      })
    })

    input.write(originalBuffer)
    input.end()
  } else {
    params.push('--resize', this.width + 'x' + this.height)
  }
  var context = new Gifsicle(params)

  if (!input) {
    context.write(originalBuffer)
    context.end()
    cb(null, context)
  }
} 
