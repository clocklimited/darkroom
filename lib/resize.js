...
function getGifsicleContext (cb) {
  ...
  if (this.mode === 'cover') {
    return gm(originalBuffer).size(function(err, originalSize) {
      if (err) {
        return cb(err)
      }
      getGifsicleCoverResize.call(this, params, originalBuffer, originalSize, newSize, cb)
    }.bind(this))
...
function getGifsicleCoverResize(params, originalBuffer, originalSize, newSize, cb) {
  // Cropping takes place before any rotation, flipping, resizing, or positioning.
  // This ensures the resize happens first
  var dimens = calculateResize(originalSize, newSize)
    , buffers = []
...
function getGmContext (imageFileType) {
  function setTypeSpecificOptions (context, format, inputFileType, outputFileType) {
    if (format) context.setFormat(format)
    // keep pngs lossless
    if (format.toLowerCase() === 'webp' && inputFileType === 'png') context.define('webp:lossless=true')
  ...
  setTypeSpecificOptions(context, this.format, imageFileType.ext, outputFileType)
